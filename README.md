...
Calculate total number of units sold per store.

```sql
SELECT ST.STORE_NAME, SUM(S.QUANTITY) AS TOTAL_NUM_SOLD
FROM SALES S
...
```sql
SELECT TO_CHAR(SALE_DATE, 'FMMonth-YYYY') AS MONTH, SUM(QUANTITY) AS TOTAL_NUMBER_SOLD
FROM SALES
...
```sql
SELECT COUNT(*) AS stores_without_warranty
FROM STORES st
WHERE NOT EXISTS (
...
### 5️⃣ Percentage of “Warranty Void” Claims

```sql
SELECT ROUND(
    COUNT(*)::NUMERIC / (SELECT COUNT(*) FROM WARRANTY)::NUMERIC * 100, 2
) AS PERCENTAGE
FROM WARRANTY
...
### 6️⃣ Store with Highest Total Units Sold (Last Year)

```sql
SELECT STORE_ID, SUM(QUANTITY) AS TOTAL_SALE
FROM SALES
...
### 7️⃣ Warranty Claims Filed in 2020

```sql
SELECT EXTRACT(YEAR FROM CLAIM_DATE) AS YEAR, COUNT(*) AS TOTAL_CLAIM
FROM WARRANTY
...
### 8️⃣ Best Selling Days by Store

```sql
SELECT * FROM (
    SELECT STORE_ID, TO_CHAR(SALE_DATE, 'DAY') AS DAY,
...
### 9️⃣ Least Selling Product per Country & Year

```sql
SELECT * FROM (
    SELECT P.PRODUCT_NAME, ST.COUNTRY, EXTRACT(YEAR FROM S.SALE_DATE) AS YEAR,
